...
{
	// nothing here, it's all done by I_JS_InitSound
...
static void I_JS_PlaySong(void *handle, boolean looping)
{
	...
	if (retval < 0) {
		fprintf(stderr, "Error playing midi: %d\n", retval);
	}
}
...
// Is the song playing?
static boolean I_JS_MusicIsPlaying(void)
{
	if (!music_initialized)
	{
		return false;
...
{
	// nothing here, js takes care of it
}
